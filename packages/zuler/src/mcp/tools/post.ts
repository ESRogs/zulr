...
export function registerPostTool(server: McpServer, ctx: ToolContext): void {
  ...
  server.registerTool(
    ...
    {
      description:
        'Send a Zulip message. For DMs, provide "to" as a user ID, name, or email. For channel messages, provide "channel" and "topic". To @-mention someone, use @**full name** (e.g. @**scout**). Note: you must read any unread messages in the target topic/DM before posting (use the read or catch-up tool first).',
