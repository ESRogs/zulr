...
Base64 :: [].{
	...
	encode_str : Str -> Str
	encode_str = |s|
		Base64.encode(s.to_utf8())

	## Encode raw bytes as base64.
	encode : List(U8) -> Str
	encode = |bytes| {
		out = Base64.encode_chunks(bytes, [])
	...
	encode_chunks = |bytes, acc|
		match bytes {
			...
			[a] => {
				...
				acc.append(Base64.char_at(i0)).append(Base64.char_at(i1)).append(61).append(61)
			}
			[a, b] => {
				i0 = a.shift_right_by(2)
				i1 = a.bitwise_and(3).shift_left_by(4).bitwise_or(b.shift_right_by(4))
				i2 = b.bitwise_and(15).shift_left_by(2)
				acc.append(Base64.char_at(i0)).append(Base64.char_at(i1)).append(Base64.char_at(i2)).append(61)
			}
			[a, b, c, .. as rest] => {
				i0 = a.shift_right_by(2)
				i1 = a.bitwise_and(3).shift_left_by(4).bitwise_or(b.shift_right_by(4))
				...
				i3 = c.bitwise_and(63)
				next = acc.append(Base64.char_at(i0)).append(Base64.char_at(i1)).append(Base64.char_at(i2)).append(Base64.char_at(i3))
				Base64.encode_chunks(rest, next)
			}
		}
