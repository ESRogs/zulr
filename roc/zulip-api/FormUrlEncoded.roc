...
FormUrlEncoded :: [].{
	...
	encode_value = |s| {
		out = s.to_utf8().fold([], encode_byte)
	...
	encode_byte = |acc, b|
		if is_unreserved(b) acc.append(b)
		else if b == 32 acc.append(43) # space -> '+'
		else acc.append(37).append(hex_digit(b.shift_right_by(4))).append(hex_digit(b.bitwise_and(15)))
